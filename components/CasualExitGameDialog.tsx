      ...
      <section className="w-full max-w-lg rounded-2xl border border-[var(--color-panel-border)] bg-[var(--color-panel)] p-5 text-[var(--color-text-main)] shadow-2xl">
          ...
          <>
            ...
            <div className="mt-5 grid gap-3">
              <button
                ...
                className="rounded-xl border border-[var(--color-primary)]/50 bg-[var(--color-primary)]/10 px-4 py-4 text-left hover:bg-[var(--color-primary)]/15"
              >
              ...
              <button
                ...
                className="rounded-xl border border-[var(--color-danger)]/40 bg-[var(--color-danger)]/10 px-4 py-4 text-left hover:bg-[var(--color-danger)]/15"
              >
              ...
              <button
                type="button"
                onClick={onCancel}
                className="rounded-xl border border-[var(--color-panel-border)] px-4 py-3 font-black hover:bg-[var(--color-panel-soft)]"
              >
                Cancel
              </button>
            </div>
        ...
        {mode === "pause" && (
          <>
            ...
            <p className="mt-2 text-sm text-[var(--color-text-muted)]">
              Give this game a name so it is easy to find later.
            </p>
            <label className="mt-4 block text-sm font-bold">
              ...
              <input
                ...
                value={pauseName}
                maxLength={100}
                onChange={(event) => setPauseName(event.target.value)}
            ...
            {isAtLimit && (
              <div className="mt-4 rounded-xl border border-[var(--color-warning)]/50 bg-[var(--color-warning)]/10 p-4">
                ...
                <p className="mt-1 text-sm text-[var(--color-text-muted)]">
                  Delete one below before saving this game. Nothing will be overwritten automatically.
                </p>
                  ...
                  {games.map((game) => (
                    <div key={game.id} className="rounded-lg border border-[var(--color-panel-border)] bg-[var(--color-panel)] p-3">
                      ...
                      <div className="mt-1 text-xs text-[var(--color-text-muted)]">
                        {game.gameLabel} · {new Date(game.pausedAt).toLocaleString()}
                      </div>
                      {pendingDeleteId === game.id ? (
                        <div className="mt-2 flex flex-wrap gap-2">
                          <button
                            ...
                            className="rounded-lg bg-[var(--color-danger)] px-3 py-2 text-xs font-bold text-white hover:bg-[var(--color-danger-hover)]"
                          >
                            Confirm Delete
                          </button>
                          <button
                            ...
                            className="rounded-lg border border-[var(--color-panel-border)] px-3 py-2 text-xs font-bold"
                          >
                            Cancel
                          </button>
                        </div>
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-3">
              ...
              <button
                type="button"
                disabled={isAtLimit || !pauseName.trim()}
                onClick={() => onPause(pauseName.trim())}
                className="rounded-xl bg-[var(--color-primary)] px-5 py-3 font-black text-white hover:bg-[var(--color-primary-hover)] disabled:cursor-not-allowed disabled:opacity-40"
              >
                Save & Pause
              </button>
            </div>
        ...
        {mode === "discard" && (
          <>
            ...
            <p className="mt-2 text-sm text-[var(--color-text-muted)]">
              This permanently removes the current casual game. Its partial turns, scores, and result will not be added to completed-game statistics or history.
            </p>
            <div className="mt-5 flex flex-wrap justify-end gap-3">
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={onDiscard}
                className="rounded-xl bg-[var(--color-danger)] px-5 py-3 font-black text-white hover:bg-[var(--color-danger-hover)]"
              >
                Discard Game
              </button>
            </div>
