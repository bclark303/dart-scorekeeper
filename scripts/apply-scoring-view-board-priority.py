...
replace_once(
    'shrink-0 rounded-2xl border border-white/20 bg-neutral-900 px-4 py-2 shadow-2xl',
    'shrink-0 rounded-xl border border-white/20 bg-neutral-900 px-3 py-1.5 shadow-2xl',
    'Scoring View header shell',
)
replace_once(
    'truncate text-2xl font-black leading-tight sm:text-3xl',
    'truncate text-xl font-black leading-tight sm:text-2xl',
    'Scoring View thrower heading',
)
replace_once(
    'text-4xl font-black leading-none text-white sm:text-5xl',
    'text-3xl font-black leading-none text-white sm:text-4xl',
    'Scoring View score heading',
)
replace_once(
...
replace_once(
    ...
    'Scoring View side panel',
)

# Scope the inner side-panel content so it can scroll without moving the pinned utility controls.
...
text = text[:inner_index] + text[inner_index:].replace(
    '<div className="grid gap-2">',
    '<div className="grid content-start gap-1.5 overflow-y-auto pr-0.5">',
    1,
)

# Add compact, pinned utility controls inside the landscape side rail.
...
# Keep the bottom utility bar only for narrow/portrait layouts, and make it compact.
replace_once(
    'flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-white/20 bg-neutral-900 p-2 shadow-2xl',
    ...
    'narrow-layout bottom toolbar',
)
replace_once(
    'flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-xs font-bold text-white/80 hover:bg-white/10',
    'flex min-h-9 cursor-pointer items-center justify-center gap-1 rounded-md border border-white/15 bg-white/5 px-2 py-1 text-xs font-bold text-white/80 hover:bg-white/10',
    'narrow auto button',
)
...
entry = '- Rebalanced Scoring View to prioritize dartboard size: landscape utility controls are pinned in a narrow side rail instead of consuming dartboard height, with a tighter header and control rail.\n'
