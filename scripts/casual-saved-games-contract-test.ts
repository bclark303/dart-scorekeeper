...
for (let index = 1; index <= MAX_PAUSED_CASUAL_GAMES; index += 1) savePausedCasualGame(game(index), storage);
const saved = listPausedCasualGames(storage);
assert.equal(saved.length, MAX_PAUSED_CASUAL_GAMES);
assert.equal(saved[0]?.id, "game-5");
assert.throws(() => savePausedCasualGame(game(6), storage), /Delete one before saving another/);
const updated = savePausedCasualGame({ ...game(3), name: "Renamed Game", pausedAt: 2_000 }, storage);
